              ...
              {totalSteps > 0 && (
                <div className="mt-4 pt-3 border-t border-white/10">
                  ...
                  <div className="w-full bg-white/10 rounded-full h-1.5 mt-2">
                    <div 
                      className="bg-gradient-to-r from-primary to-gold h-1.5 rounded-full transition-all duration-500" 
            ...
            <div className="flex justify-between border-t border-white/10 p-3 bg-white/5">
              {messages.length > 1 && (
                <div className="flex space-x-2">
                  <Button
                    onClick={() => setCurrentMessage(prev => Math.max(0, prev - 1))}
                    disabled={currentMessage <= 0}
                    size="sm"
                    variant="ghost"
                    className="h-8 px-2 text-white/70"
                  >
                    &lt;
                  </Button>
                  <Button
                    onClick={() => setCurrentMessage(prev => Math.min(messages.length - 1, prev + 1))}
              ...
              <Button 
                ...
                className="ml-auto h-8 text-xs text-white/70"
              >
                Cerrar
              </Button>
            </div>
